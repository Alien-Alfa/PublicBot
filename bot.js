...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('open', async () => {
        ...
        if (config.WORKTYPE == 'public') {
            ...
            else { await conn.sendMessage(conn.user.jid, '*Thanks for using Farhan-Dqz 🧞‍♂️*\n```Type``` *.help* ```For Commands```', MessageType.text);
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                    await conn.sendMessage(
                        ...
                        try {
                            ...
                            if (error.message.includes('429')) return;
                                ...
                                if (error.message.includes('URL')) {
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '*🙇🏻 Error Entha manassilayilla macha😔!*' +
